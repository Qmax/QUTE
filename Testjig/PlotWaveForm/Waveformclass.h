...
public:
    ...
    void LoadPlotterData(QString pStrFileName,stWaveData *pObjWaveData)
    {
        ...
        qDebug() << "Count:"<<m_strWaveData.count();
    }
...
public:
    ...
    virtual void GeneratePlotterPoints(short pChannelValue)
    {
        ...
        for(int l_nIndex=0;l_nIndex<m_strWaveData.count();l_nIndex++)
        {
           ...
           // qDebug()<< "X Point:"<<l_nXPoint<<"Y Point:"<<l_nYPoint;
        }
    ...
    {
    	qDebug()<<"Resolution:"<<m_objWaveData->m_nResolution;
        bool ok = true;
        double l_nVref = 0.7;
 		return ((2*(pStrData.toInt(&ok,16)*l_nVref))/ (pow(2,m_objWaveData->m_nResolution)-1)) - l_nVref;
    }

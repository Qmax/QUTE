...
struct DSOMeasurements
{
public:
	DSOMeasurements()
	{

		m_nPositivePeakVoltage=0.0;
		m_nNegativePeakVoltage =0.0;
		m_nMaxPeak =0.0;m_nMinPeak=0.0;
		m_nTenPercentPeak = m_nNientyPercenPeak=0.0;
		m_nX1Pos = m_nX2Pos = 0;
	}

	void setTenPercentPeak(double pValue) { m_nTenPercentPeak = pValue;}
	void setNientyPercentPeak(double pValue){ m_nNientyPercenPeak = pValue;}

	double getTenPercentPeak() { return m_nTenPercentPeak;}
	double getNinentyPercentPeak() { return m_nNientyPercenPeak;}
	void setMaxPeak(double pValue) { m_nMaxPeak = pValue;}
	double getMaxPeak() const {return m_nMaxPeak;}

	void setMinPeak(double pValue){ m_nMinPeak = pValue;}
	double getMinPeak() const { return m_nMinPeak;}
	void setPostivePeakVoltage(double pValue)
	{
		m_nPositivePeakVoltage = pValue;
	}
	void setNegativePeakVoltage(double pValue)
	{
		m_nNegativePeakVoltage = pValue;
	}
	double getPositvePeakVoltage() const
	{
		return m_nPositivePeakVoltage;
	}
	double getNegativePeakVoltage() const
	{
		return m_nNegativePeakVoltage;
	}

	void setX1Pos(unsigned int pValue ){ m_nX1Pos = pValue;}
	void setX2Pos(unsigned int pValue ){ m_nX2Pos = pValue;}
	unsigned int getX1Pos() { return m_nX1Pos;}
	unsigned int getX2Pos() { return m_nX2Pos;}
protected:
	double m_nPositivePeakVoltage;
	double m_nNegativePeakVoltage;
	double m_nMaxPeak,m_nMinPeak;
	double m_nTenPercentPeak,m_nNientyPercenPeak;
	unsigned int m_nX1Pos,m_nX2Pos;
...
public:
	...
	void setDSOBTUR(unsigned int pBTUR)
	{
     ...
     QMap<int,QList<double> > getYVectorPoints(int pNo) const
	{
        if(pNo == 0)
		{
...
//			l_nSamplesperPixel = 1.0;
		...
		if(m_eSamplesType == NORMAL || m_eSamplesType == MIN_MAX || m_eSamplesType == AVERAGE)
			l_nSamplesperPixel = 1.0;
		int l_nMinMaxCounter=0;
		//qDebug()<<l_nSamplesperPixel;
		m_objMinMaxPoints.clear();
	   for(unsigned short int l_nAddrIndex=0;l_nAddrIndex<pRectWidth;l_nAddrIndex++)
	   {
		   ...
		   for(int l_nFileCount=0;l_nFileCount<l_nSamplesperPixel;l_nFileCount++)
		   {
			   if(pSelFIFO == R1FIFO )
			   {
				 ...
				 m_objMinMaxPoints.insert(l_nMinMaxCounter++,convertFIFOData(pLstOdd.value(l_nGCounter)));
			   }
...
//		   }
	   ...
	   fillDSOMeasurements(pSelFIFO);
}
	void fillDSOMeasurements(DSO_RECEIVE_FIFO pSelFIFO)
	{
		if(pSelFIFO ==R1FIFO)
		{

			findMinMax(m_objMinMaxPoints);
			//findZeroCrossingPoints(m_objMinMaxPoints,pSelFIFO);
			//m_objCH1Measurements->setAmplitude_Vptop(m_objCH1Measurements->get)
			//qDebug()<<"RawMaxPoint:"<<m_nMaxPoint<<"RawMinPoint:"<<m_nMinPoint<<"VoltPerDiv:"<<m_nVolt<<"FS Settings:"<<m_nFSSettings;
			//qDebug()<<"MaxPoint:"<<m_nMaxPoint/((m_nVolt * 8.0) / m_nFSSettings)<<"MinPoint:"<<m_nMinPoint/((m_nVolt * 8.0) / m_nFSSettings);
			double l_nMaxValue = (m_nMaxPoint/((m_nVolt * 8.0) / m_nFSSettings));
			double l_nMinValue = (m_nMinPoint/((m_nVolt * 8.0) / m_nFSSettings));
			m_objCH1Measurements.setMinPeak(l_nMinValue);
			m_objCH1Measurements.setMaxPeak(l_nMaxValue);
			double percentValue =0.1;
			if(m_nMinPoint <0)
				percentValue = l_nMinValue*0.9;
			else
				percentValue = l_nMaxValue*0.1;
			//qDebug()<< "Min 10 Percent:"<<m_nMinPoint*0.1<<m_nMinPoint<<percentValue;
			//qDebug()<< "Max 90 Percent:"<<m_nMaxPoint*0.9<<m_nMaxPoint;
			for(unsigned int l_nIndex=0;l_nIndex<m_objMinMaxPoints.count();l_nIndex++)
			{
				if(m_objMinMaxPoints.value(l_nIndex) > m_nMinPoint*0.9 && m_objMinMaxPoints.value(l_nIndex) <=m_nMinPoint*0.88)
				{
					if(l_nIndex> 100 && l_nIndex<150 )
					{
						qDebug()<< "X1 Pos:"<<l_nIndex<<l_nMinValue;
						m_objCH1Measurements.setX1Pos(l_nIndex);
					}

				}
				if(m_objMinMaxPoints.value(l_nIndex) > m_nMaxPoint*0.88 && m_objMinMaxPoints.value(l_nIndex) <=m_nMaxPoint*0.9)
				{
					if(l_nIndex> 150 && l_nIndex<200 ){
						qDebug()<< "X2 Pos:"<<l_nIndex<<l_nMaxValue;
						m_objCH1Measurements.setX2Pos(l_nIndex);
					}
				}

			}
			m_objCH1Measurements.setTenPercentPeak(percentValue);
			m_objCH1Measurements.setNientyPercentPeak(l_nMaxValue*0.9);
			//qDebug()<<"Zero Crossing Points CH1:"<<m_nCH1ZeroCrossPoints;
			//qDebug()<<"Maximum Value:"<<l_nMaxValue;
			//qDebug()<<"Minimum Value:"<<l_nMinValue;
		}
		m_objMinMaxPoints.clear();
	}
...
double	convertFIFOData(unsigned char pStrData)
{
	...
	DSOMeasurements m_objCH1Measurements;
	protected:
    ...
    QList<double> m_objMinMaxPoints;
    QList<int> m_nMinPosition;

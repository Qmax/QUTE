    ...
    : QWidget(parent)
{
        ...
        readCalibrationFile();
        this->setWindowTitle("DMM Display Calibration Panel");
        connect(parent,SIGNAL(DMM2GCalib(double,QString)),this,SLOT(DMM2GCalib(double,QString)));

}
void DACCalibration::DMM2GCalib(double value,QString com){
...
//    connect(m_objLE_RxedValue, SIGNAL(focussed(bool)), this, SLOT(callRxedValueEdit()));
    ...
    m_nslope=1.0;m_nconstant=0.0;


    ui.ICM->clear();
    ui.ICM->insertItem(0,"50E");
    ui.ICM->insertItem(1,"500E");
    ui.ICM->insertItem(2,"5KE");
    ui.ICM->insertItem(3,"50KE");
    ui.ICM->insertItem(4,"500KE");
    ui.ICM->insertItem(5,"5ME");
    ui.ICM->insertItem(6,"50ME");
...
//            l_strFileName = "DMMCalibration.xml";
    ...
    int ICM_Current_Parameter=ui.comboBox->currentIndex();
     ...
     for (int l_nGainIndex = 0; l_nGainIndex<ui.comboBox->count(); l_nGainIndex++) {
         ...
         for (int l_nValueIndex = 0; l_nValueIndex<(ui.ICM->count()); l_nValueIndex++) {
                 ...
                 if(ui.comboBox->itemText(l_nGainIndex)=="ICM-C")
                    ...
                    objCalibration.writeAttribute("Slope", QString::number(m_nSlope[l_nGainIndex][l_nValueIndex],'f',15));
                    qDebug()<<"Gain"<<QString::number(m_nSlope[l_nGainIndex][l_nValueIndex],'f',15);;

                    objCalibration.writeAttribute("Constant", QString::number(m_nConstant[l_nGainIndex][l_nValueIndex],'f',15));
                    qDebug()<<"Constant"<<QString::number(m_nConstant[l_nGainIndex][l_nValueIndex],'f',15);
    ...
    ui.comboBox->setCurrentIndex(ICM_Current_Parameter);
    on_comboBox_currentIndexChanged(ICM_Current_Parameter);

    ui.ICM->setCurrentIndex(ICM_Current_Index);
}
void DACCalibration::readCalibrationFile(){
    ...
    QFile *xmlFile= new QFile(l_strFileName);
            ...
            short int index,RIndex,LIndex,CIndex;
            index=0; RIndex=0; LIndex=0; CIndex=0;
...
void DACCalibration::callActualValueEdit(){
    if (m_objLE_ActualValue->hasFocus()) {
    	if(ui.comboBox->currentText()=="R-2W"||ui.comboBox->currentText()=="R-4W")
               openNumKBPanelminmax(1, 'R',50000000, 0);
...
void DACCalibration::on_comboBox_currentIndexChanged(int index)
{
    ...
    case 2:
        ui.ICM->clear();
        ui.ICM->insertItem(0,"50mV");
        ui.ICM->insertItem(1,"500mV");
        ui.ICM->insertItem(2,"5V");
        ui.ICM->insertItem(3,"50V");
        ui.ICM->insertItem(4,"500V");
        ui.ICM->insertItem(4,"750V");
...
//			if(ui.ICM->currentText()=="200mV"||ui.ICM->currentText()=="2mA"||ui.ICM->currentText()=="20mA"||ui.ICM->currentText()=="200mA"){
//				   m_nSlope[ui.comboBox->currentIndex()][ui.ICM->currentIndex()]=m_nslope*1000;
//			}
//			else{

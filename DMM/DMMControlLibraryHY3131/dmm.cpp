...
DMM::DMM(QWidget *parent) :
	...
	DMMGraph->setGraphGeometry(0,424,403,180);
...
void DMM::configGraphData(){
	...
	double _10P = (rang*10)/100;

	rang = rang + _10P;

	if(Flag.dcFlag==1)
		DMMGraph->setGraphRange("Time",0,15,ui->label->text()+" Range",rang*-1,rang);
	else if(Flag.diodeFlag==1)
		DMMGraph->setGraphRange("Time",0,15,"mV",0,2000);
	else
		DMMGraph->setGraphRange("Time",0,15,ui->label->text()+" Range",0,rang);
}
...
void DMM::buttonPressed(int pPressed) {
	...
	case 4:
		...
		else if(dis->getRange()==3 || dis->getRange()==5){
	...
	if (Flag.buzzerFlag == 0 || Flag.continuityFlag == 0){
		qDebug()<<"Flag.buzzerFlag:"<<Flag.buzzerFlag<<"Flag.continuityFlag:"<<Flag.continuityFlag;
		Beep(false);
	}
}

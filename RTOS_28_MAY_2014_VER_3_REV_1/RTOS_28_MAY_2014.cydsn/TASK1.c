...
void k1_k2_check()							//	Subroutine to Check Connection of Probe 1 & 2
{
	...
	ucKey2Read  = CyPins_ReadPin(K2_SENSE_P4_1);
}
...
void switching_main(unsigned char *cmd)		// Main Relay Switching Subroutine
{
	...
	switch(*cmd){
		...
		case 0xF1:										//	VI-CH_1
			...
			UART_PutString("VICH1");			//	STATUS ACKNOWLEDGEMENT to uP	//
			while(!(UART_ReadTxStatus() & UART_TX_STS_FIFO_EMPTY)){;}
		...
		case 0xF2:										//	VI-CH_2
            ...
            CyDelayUs(100);
            CyPins_SetPin(PC_VI_2SEL1_P2_4);
            
			UART_PutString("VICH2");			//	STATUS ACKNOWLEDGEMENT to uP	//
			while(!(UART_ReadTxStatus() & UART_TX_STS_FIFO_EMPTY)){;}
...
/*			UART_PutString("*");
		...
		case 0x03:										//	ICM Select
			...
			UART_PutString("ICMSL");			
			while(!(UART_ReadTxStatus() & UART_TX_STS_FIFO_EMPTY)){;}
			
			break;
		case 0xFC:										//	fly channel select

			//Relays_Init();				
            
            CyPins_SetPin(MUXASEL_P2_1);

			UART_PutString("FLYCH");		
			while(!(UART_ReadTxStatus() & UART_TX_STS_FIFO_EMPTY)){;}
		...
		case 0x06:										//	Function Generator
			...
			UART_PutString("FGSEL");			//	STATUS ACKNOWLEDGEMENT to uP	//
			while(!(UART_ReadTxStatus() & UART_TX_STS_FIFO_EMPTY)){;}
		...
		case 0x09:										//	Ex-MUX I/F	//
            CyPins_SetPin(MUXSEL_B0_P3_4);
            CyPins_SetPin(MUXSEL_B1_P3_5);
...
//			while(!(UART_ReadTxStatus() & UART_TX_STS_FIFO_EMPTY)){;}
		...
		case 0x46:										//	Read SRC IMP Port Status	//
            ...
            if (CyPins_ReadPin(BSRC_IMP_SEL_P6_3)) 
                RLY_PRT_STS = RLY_PRT_STS | 0x20;
            else
                RLY_PRT_STS = RLY_PRT_STS & 0xDF;            
            
                RLY_PRT_STS	= RLY_PRT_STS & 0x3F;
    			UART_ClearTxBuffer();
    			UART_WriteTxData(RLY_PRT_STS);
    			while(!(UART_ReadTxStatus() & UART_TX_STS_FIFO_EMPTY)){;}
					
			break;   
		case 0x47:										//	SRC IMP ENABLE
		

			CyPins_SetPin(BSRC_IMP_SEL_P6_3);
            
			UART_PutString("SRCEN");			//	STATUS ACKNOWLEDGEMENT to uP	//
			while(!(UART_ReadTxStatus() & UART_TX_STS_FIFO_EMPTY)){;}
			
		
			break;                
		case 0x48:										//	SRC IMP DISABLE
            
			CyPins_ClearPin(BSRC_IMP_SEL_P6_3);
			
			UART_PutString("SRCDS");			//	STATUS ACKNOWLEDGEMENT to uP	//
			while(!(UART_ReadTxStatus() & UART_TX_STS_FIFO_EMPTY)){;}

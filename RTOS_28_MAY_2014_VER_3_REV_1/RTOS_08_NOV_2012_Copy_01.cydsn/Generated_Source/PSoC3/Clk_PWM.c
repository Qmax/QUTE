...
{
    if (Clk_PWM_CLKEN & Clk_PWM_CLKEN_MASK)
    {
...
*
*******************************************************************************/
void Clk_PWM_SetModeRegister(uint8 clkMode) 
{
    Clk_PWM_MOD_SRC |= clkMode & Clk_PWM_MODE_MASK;
}
...
*
*******************************************************************************/
void Clk_PWM_ClearModeRegister(uint8 clkMode) 
{
    Clk_PWM_MOD_SRC &= ~clkMode | ~Clk_PWM_MODE_MASK;
}
...
{
    return Clk_PWM_MOD_SRC & Clk_PWM_MODE_MASK;
}
...
void Clk_PWM_SetSourceRegister(uint8 clkSource) 
{
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        Clk_PWM_MOD_SRC |= CYCLK_SSS;
        Clk_PWM_MOD_SRC =
            (Clk_PWM_MOD_SRC & ~Clk_PWM_SRC_SEL_MSK) | clkSource;
    }

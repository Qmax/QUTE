...
unsigned char PTDMMLibInterface::calibrateDMM(){
	...
	//qDebug()<<"Receive Calibration Slope:"<<calib_value.gain<<"Constant:"<<calib_value.constant;
}
void PTDMMLibInterface::ApplyDACOffset(bool apply){
	...
	if(iamDMM==false){
		...
		if(psuedoMode==true)
			...
			l_nAD5318DW.m_bUniPolor = false;		m_objAD5318Component->configureADC5318CW();
...
double PTDMMLibInterface::GetADCVoltage(unsigned int type,unsigned int Gain){
	...
	if(Gain==1){
		if(iamDMM==false)
			if(psuedoMode==true)
				m_nConfigurationRegister=0x4010;
			else
		...
		if(iamDMM==false)
			m_nConfigurationRegister=0x4013;
		else
	...
	if(psuedoMode==true){
		l_nFactor1 = (((AIN * 2.048) / (pow(2, 23)))-2.048) / 1;
		l_nAIN=l_nFactor1;
	}else{
...
double PTDMMLibInterface::displayResistance(unsigned int rValue)
{
	...
	Resistance =  Resistance1;//(Resistance1 - Resistance2)/2;
	//    qDebug()<<"Resistance-1:"<<qSetRealNumberPrecision(7)<<Resistance1;
	//    qDebug()<<"Resistance-2:"<<qSetRealNumberPrecision(7)<<Resistance2;
	...
	if(psuedoMode==true){
		if(rValue==R200E) Resistance = Resistance / 1 ;
		else if(rValue==R2E) Resistance = Resistance / 1 ;
		else if(rValue==R200mE) Resistance = Resistance / 1 ;
	}
	return Resistance;
...
double PTDMMLibInterface::MeasureResistance(unsigned int rValue,unsigned int rType){
	...
	if(rValue==R200mE||rValue==R2E||rValue==SLR200E){
		...
		if(psuedoMode==true){
			IAppCard->writeRegister(0x0000,0x0048);
			IAppCard->writeRegister(0x0034,0x0036);
		}
		else{
	...
	if(rValue==SLR200E){
		if(psuedoMode==true){
			IAppCard->writeRegister(0x0001,0x0046);
			usleep(10000);
		}
		IAppCard->writeRegister(0x000F,0x0016);
		test=ICALIB->SerialDACCalibration(SLR200E);
		...
		if(psuedoMode==true){
			IAppCard->writeRegister(0x0002,0x0046);
			usleep(10000);
		}
		IAppCard->writeRegister(0x000F,0x0016);
		test=ICALIB->SerialDACCalibration(R2E);
		...
		if(psuedoMode==true){
			IAppCard->writeRegister(0x0004,0x0046);
			usleep(10000);
		}
		IAppCard->writeRegister(0x000F,0x0016);
		test=ICALIB->SerialDACCalibration(R200mE);
	...
	if(rValue==SLR200E){
		...
		m_objAD7190->writeADCGPConRegister(1,0x31);
	}
	else if(rValue==R2E){
		l_dbAIN=GetADCVoltage(typeDC,1);
		...
		m_objAD7190->writeADCGPConRegister(1,0x31);//GPCON - 1
	}
	//-------------------------------------------------------------------------------------

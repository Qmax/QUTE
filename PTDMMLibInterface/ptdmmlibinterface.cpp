...
double PTDMMLibInterface::GetADCVoltage(unsigned int type,unsigned int Gain){
    ...
    short m_nADC=1;
    unsigned int m_nModeRegister=0x80060,m_nConfigurationRegister;
...
//        qDebug("ADC Data : %x",AIN);
        ...
        if(rValue==R200E||rValue==R2K||rValue==R20K||rValue==R200K){
            l_nFactor1 = (AIN/pow(2,23))-1;
        ...
        else if(rValue==R2M||rValue==R20M){//||rValue==SLR200E){
            l_nFactor1 = (AIN/pow(2,23))-1;
            l_nFactor2 = (1.0 / 2.048);
            l_nAIN = l_nFactor1 / l_nFactor2;
            gainValue=1;
...
/*
    ...
    else if (rValue==R2K)        Resistance = c_nAIN / 0.0001;
    else if (rValue==R20K)       Resistance = c_nAIN / 0.00001;
    else if (rValue==R200K)      Resistance = c_nAIN / 0.000001;
    else if (rValue==R2M)        Resistance = c_nAIN / 0.0000005;

    else if (rValue==R20M){
        Resistance = (c_nAIN*10000000)
                     /
                     ((0.00000025*10000000)-c_nAIN);
    }
*///commented on 12062014

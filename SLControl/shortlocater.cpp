...
void ShortLocater::customEvent(QEvent *e) {
	...
	if (e->type() == ((QEvent::Type) 1234)) {
		...
		IBackPlane->writeBackPlaneRegister(0x0000, 0x0020);
		IBackPlane->writeBackPlaneRegister(0x0100, 0x001E);

		IGPIOEvent->BlockSig(true);
		m_nKeyCode = IPsoc->embeddedProbeStatus();
		doKeyFunction(m_nKeyCode);
		IGPIOEvent->BlockSig(false);

		IBackPlane->writeBackPlaneRegister(0x0100, 0x0020);
	}
...
{
	if (pKeyCode == 0x8b || pKeyCode == 0x0b || pKeyCode == 0x8D || pKeyCode == 0xD)
	{
		if(msgBoxLive==true)
	...
	else if (pKeyCode == 0x8a || pKeyCode == 0x0a || pKeyCode == 0x8E || pKeyCode == 0xE)
	{
		if(OffsetFlag!=true){
			if (AutoFlag == false) {
				if (rangeFlag == "200E")
	...
	else if (pKeyCode == 0x89 || pKeyCode == 0x09 || pKeyCode == 0x8F || pKeyCode == 0xF)
	{
...
void ShortLocater::checkProbeConnect() {
	...
	if((IPsoc->readSerial()&0x08)!=0x08)
		checkPrbStatus=showMessageBox(true,false,"Connect Probe","OK","Cancel");
}
...
void ShortLocater::on_offset_clicked() {
	...
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		if (OffsetFlag == false) {
			tempBuzStatus=BuzzerFlag;	BuzzerFlag=false;
			OffsetFlag = true;
			ui.offset->setStyleSheet(
					"QPushButton {color:white;border: 1px solid #2D5059;border-radius: 20px;background-color: qlineargradient(x1: 0, y1: 1, x2: 1, y2: 0,stop: 0 #e59244, stop: 0.6 #ffa500 , stop:1 #FFFFFF);font:bold;}");

			prevRangeFlag=rangeFlag;
				...
				if (rangeFlag != "200mE")
					do {
						...
						if (readData > 0.001 || readData < -0.001) {
							...
							if(readData2>2.048){
								showMessageBox(true,false,"Unable to Offset Try Again");
								IAppCard->writeRegister(0x0000,0x0048);
								status4=false;

								OffsetFlag = false;

								if (prevRangeFlag == "200E") {
									on_r200But_clicked();
								} else if (prevRangeFlag == "2E") {
									on_r2EBut_clicked();
								} else if (prevRangeFlag == "200mE") {
									on_r200mEBut_clicked();
								}
...
//							usleep(5000);
			...
			QApplication::processEvents();
			qDebug()<<"################prevRangeFlag:"<<prevRangeFlag;
			if (prevRangeFlag == "200E") {
				on_r200But_clicked();
			} else if (prevRangeFlag == "2E") {
				on_r2EBut_clicked();

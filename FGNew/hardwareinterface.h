...
enum DRIVE_MODE{
    SINGLE_W,CONTINUOUS_W,BURST_W,STARTDRIVE,STOPDRIVE
};
...
public:
    ...
    HardwareInterface(){
    	...
    	m_objFGData = new FGModel(this);
    	l_objDriveData = new Drive2Data;
...
//     				//qDebug() << "FILE Data:" << l_nFileData;
     ...
     void setBurstRate(double burstRate){
    	 double l_nTime=(burstRate/(100e-9));
    	 m_nWDP=(unsigned int)l_nTime;
     ...
     void InteractFgModelData(FGModel* data){
    	 m_objFGData=data;
...
/*    	 double l_dTime[6]=		{	0.0000001,	0.000001,0.00001,	0.0001,	0.001,	0.01	};//100ns,1us,10us,100us,1ms,10ms
    	 ...
    	 unsigned int m_uBDU[6]={	0x1,		0x1,	 0x10,		0x10,	0x100,	0x1000	};*/
     ...
     void setPeriod(double pdValue){
    	 setFrequency(1/pdValue);
     }
     double getPeriod(){
    	 return 0;
     }
     void setHighImpedance(bool state){
   		 m_bHighImpedance=state;
...
//    		double m_nVoltRangeArray[5]={0.6,1.5,3.5,7.0,13.0};//Aug,30,2013
     ...
     void setHlevel(double hValue){
    	 m_nHLevel=hValue;
     	m_nAmplitude=m_nHLevel-m_nLLevel;
     	m_nOffset=(m_nAmplitude/ 2)+m_nLLevel;
     	setAmplitude(m_nAmplitude);
     	setOffset(m_nOffset);
     }
     double getHlevel(){
...
protected:
    ...
    double m_nOutputPhase,m_nOutputTime,m_nAmplitude,m_nOffset,m_nHLevel,m_nLLevel;
    double m_nAmplitudeIn;
    ...
    bool DDSLoop,PatternLoop,m_bHighImpedance;

    FGModel *m_objFGData;
    Drive2Data *l_objDriveData;
